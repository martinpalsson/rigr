...
class GraphicDirective(SphinxDirective):
    ...
    Output format (per issue #11):
        - ID + Title on same line at top
        - Body (graphic) on left, metadata table on right
        - All metadata rendered including level/status
    """
...
class CodeDirective(SphinxDirective):
    ...
    Output format (per issue #11):
        - ID + Title on same line at top
        - Body (code) on left, metadata table on right
        - All metadata rendered including level/status
    """
...
/* ID displayed in title (per issue #9) */
.rigr-title-id {
...
/* -----------------------------------------------------------------------------
   Content Wrapper - body on left, metadata on right (per issue #9)
   ----------------------------------------------------------------------------- */
...
/* -----------------------------------------------------------------------------
   Metadata Table - compact styling (per issue #9)
   ----------------------------------------------------------------------------- */
.rigr-metadata-table {
    width: 100%;
...
/* Clickable link references (per issue #9) */
.rigr-link-ref {
    color: var(--rigr-id-color);
...
/* -----------------------------------------------------------------------------
   Body Content - adjusted for side-by-side layout
   ----------------------------------------------------------------------------- */
...
.rst-content .rigr-metadata-wrapper table {
    margin: 0 !important;
...
.rst-content .rigr-metadata-table tbody,
.rst-content .rigr-metadata-table tgroup,
.rst-content .rigr-metadata-wrapper tbody {
    margin: 0 !important;
    padding: 0 !important;
}

/* RTD body padding override */
.rst-content .rigr-body {
    padding: 0.5em 0.75em 0.5em 1em !important;
}
